...
export class ApiservicesService {
  ...
  generateOffer(data:any){
    return this.http.post(this.x+'ctc/create-offer-letter/',data,{headers:{"Authorization":"Bearer "+localStorage.getItem('token')}})
  }
change_password(d)
{
...
savetagSelectedCandidate(formData,email){
  return this.http.post(this.x + "hr?email=" + email, formData)
}
...
getTagselectedcandidates(){
  return this.http.get(this.x +'hrselectedcandidate')
}
getTAGInternalReqs(){
...
get_selectedCadidates_Details_ById(req_id,email) {
  return this.http.get(this.x + 'selectedcandidatebyid?req_id=' + req_id​​​​​​​ +'&&email=' + email);
}
deleteSelectedCandidates(id,email){
  return this.http.delete(this.x + 'hr?req_id=' +id + '&&email=' +email)
}
feedbacks(i){
